...
The generated `index.qmd` SHALL contain YAML front matter with: `title` (from input), `date` (today's date in YYYY-MM-DD format), `description` (empty string), `bibliography: refs.bib`, and `draft: true`. The template SHALL NOT include `author` or `categories` fields — author is inherited from `posts/_metadata.yml`.

#### Scenario: Front matter content
- **WHEN** a post is scaffolded with title "My Post"
- **THEN** the `index.qmd` contains front matter with the title set to "My Post", today's date, an empty description, bibliography set to `refs.bib`, and draft set to `true`
